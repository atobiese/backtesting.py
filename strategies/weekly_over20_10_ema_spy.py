...
def get_signals():
    ...
    # eventbased signals
    def _mea_10_20_sig_next(backcandles, dfa):
        sig = 0
        upt = 0
        dnt = 0
        for i in range(0, backcandles + 1):
            if dfa.EMA10[i] >= dfa.EMA20[i]:
                upt = 1
            if dfa.EMA10[i] <= dfa.EMA20[i]:
                dnt = 1
        if upt == 1 and dnt == 1:
    ...
    # vwap_sig = [0] * len(df)
    ema_10_20_sig = [0] * (backcandles)
    nr_samples = len(df)

    for row in range(backcandles, nr_samples):
        sig = _mea_10_20_sig_next(backcandles, df[row - backcandles:row + 1])
        ema_10_20_sig.append(sig)

    df['ema_10_20_sig'] = ema_10_20_sig
    ...
    sub_indicators0 = ['KSTs_9', 'KST_10_15_20_30_10_10_10_15', 'total_signal', 'ema_10_20_sig']
